...
#include "caffe/3rdparty/dbglog.h"
...
/*
 * Get value of given entry name (float).
 * If entry is not found or has invalid float valus,
 * then value param is left untouched (good for pre-initialization
 * with defaults).
 * Return: 1=entry found, 0=not found.
 * Param section: section in which is the desired entry
 ...
 * Param value: buffer for entry value
 */
int ConfigParser_t::getValue(string section, string name, float *value) const {
    ...
    if (getValue(section, name, &val)) {
        ...
        /* try to convert string into int */
        char *endptr;
        float d = strtof(val.c_str(), &endptr);
        /* if entire string is valid */
        if (*endptr == 0) {
            /* supply value */
            if (value != NULL)
                *value = d;
            /* debug */
            DBG(DBG1, "Float value %f", d);
            return 1;
        }
    }
    /* return not found/invalid */
    DBG(DBG1, "Float value INVALID");
    return 0;
}
...
int ConfigParser_t::getValue(
            std::string section,
            std::string name, std::vector<int> *value) const
{
    std::vector<std::string> strs;
    int val_ret = getValue(section, name, &strs);
    for(int i = 0; i < strs.size(); i ++)
    {
        value->push_back(atoi(strs[i].c_str()));
    }
    return val_ret;
}
...
int ConfigParser_t::getValue(
       std::string section,
       std::string name, std::vector<float> *value) const
{
    std::vector<std::string> strs;
    int val_ret = getValue(section, name, &strs);
    for(int i = 0; i < strs.size(); i ++)
    {
        value->push_back((float)atof(strs[i].c_str()));
    }
    return val_ret;
}
